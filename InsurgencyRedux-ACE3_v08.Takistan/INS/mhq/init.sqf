...
_fnc_addTeleportPT = {
	...
	[
		...
		missionNamespace setVariable ["currentActionID",(_this select 2)];
		},_object
	] remoteExecCall ["addAction",west,true];
	_actionID = missionNamespace getVariable ["currentActionID",-1];
...
_fnc_removeTeleportPT = {
	...
	["INS_REMOVEACTION_EH",[INS_teleportFlag,_actionID]] call CBA_fnc_localEvent;
...
{
	["INS_mhq_addAction",[_x,"<t color='#ff1111'>Deploy MHQ</t>",FUNC(deployMHQ),1,"!(_target getVariable ['deployed',true]) && !(_target getVariable ['removeAction',false]) && ((speed _target) == 0)"]] call CBA_fnc_localEvent;
	["INS_mhq_addAction",[_x,"<t color='#ff1111'>Pack MHQ</t>",FUNC(packMHQ),1,"_target getVariable ['packed',false] && !(_target getVariable ['removeAction',false]) && ((speed _target) == 0)"]] call CBA_fnc_localEvent;
