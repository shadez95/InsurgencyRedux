...
[-1, {
	...
	[_side,"HQ"] sideChat "An MHQ is being deployed...";
	if ((_HQ distance player < 18) && !(_HQ getVariable ["removeAction",true])) then {
		hintSilent "MHQ is unpacking please stand back...";
	};
...
sleep _deployTime;
[-1,{params["_side"];[_side,"HQ"] sideChat "An MHQ is being deployed...";},[_side]] call CBA_fnc_globalExecute;
sleep _deployTime;

private ["_fnc_tpSquad","_fnc_arsenal"];
_fnc_tpSquad = {
	if (MHQ_tpSquad) then {
		["INS_mhq_addAction",[_this,"<t color='#c70000'>Teleport to Squad</t>", { call compile preprocessFileLineNumbers "INS\squad_teleport\teleport_squadmember.sqf"; }, 6, ""]] call CBA_fnc_globalEvent;
	};
};
_fnc_arsenal = {
	if (MHQ_arsenal) then {
...
_tpPoint = [QGVAR(add_tpPoint),[_respawnObj,format["Teleport to %1",_text]]] call CBA_fnc_globalEvent;
_HQ setVariable ["respawnPoint",_respawnObj,true];

[-1, {
	params[["_HQ",objNull],"_side"];

	[_side,"HQ"] sideChat "An MHQ has been deployed...";
	if (_HQ distance player < 30) then { hintSilent "MHQ has deployed..."; };
},[_HQ,_side]] call CBA_fnc_globalExecute;