...
[-1, {
	...
	[_side,"HQ"] sideChat "An MHQ is being packed...";
	if ((_HQ distance player < 18) && !(_HQ getVariable ["removeAction",true])) then {
		hintSilent "MHQ is packing standby...";
	};
...
[-1, {
	...
	[_side,"HQ"] sideChat "An MHQ is packed...";
	if (_HQ distance player < 18) then { hintSilent "MHQ is done packing..."; };
},[_HQ,_side]] call CBA_fnc_globalExecute;